...
public class VideoService {
    ...
    public void createTimelapseVideo(List<File> inputImageFiles, File outputVideoFile, int rate) throws IOException {
        File inputFile = File.createTempFile("f2mp4-list-", ".txt");
        try (PrintStream out = new PrintStream(new FileOutputStream(inputFile))) {
            inputImageFiles.forEach(file -> {
                out.printf("file '%s'%n", file.getAbsolutePath().replace('\\', '/'));
            });
        }
        LOGGER.info("List file created: {}", inputFile);
        ...
        for (int i = 0; i < ffmpegCommands.length; i++) {
            ...
            ffmpegCommands[i] = ffmpegCommands[i].replace(RATE, Integer.toString(rate));
        }
        OsUtil.OsCommandResult result = OsUtil.runCommandAndReadOutput(ffmpegCommands);
        if (result.code() >= 0) {
            if (outputVideoFile.length() < 100L) {
                throw new RuntimeException("ffmpeg call not successful! Empty MP4 output.");
            }
        } else {
